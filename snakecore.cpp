    ...
    private: 
        ...
        STATUS run(){
            ...
            for(int i = 0; i < 4; i++, SCORE /= 10){
                (*(this->smap)).PAINT(this->iuuid, sPOINT(12 - i*2, 39), sPOINT(12 - i*2, 39), '0' + SCORE % 10);
            }
            (*(this->smap)).PAINTMAIN();
            (*(this->smap)).PRINTSCR();
            return SUCCEED;
    ...
    public:
        ...
        STATUS MAINLOOP(int ini){
            ...
            while(1){
                ...
                hardset = (*(this->GAMEMAIN)).SHOWSCORE()<20?(*(this->GAMEMAIN)).SHOWSCORE()*10:200;
                Sleep(ini - time(NULL) + now_time - hardset);
            }
            return SUCCEED;
        }
};
